...
public class TLETest {
	...
	public static void printAllNextPasses(String tleURL) throws MalformedURLException {
		...
		if (sptList != null) {
			for (SatInfo si : sptList) {
				System.out
						.println("Next pass of satellite: " + si.getName() + newline + newline + si.getNextPassTime() + newline + newline);
			}
		}
	}

	public static void printNextPass(String tleURL, String satName) throws IllegalArgumentException, InvalidTleException,
			SatNotFoundException, MalformedURLException {
		TLEInfoPage tleInfo = new TLEInfoPage(tleURL);
	...
	public static void printNextPasses(SimpleDateFormat sdf, PassPredictor pp, int nHours) throws InvalidTleException,
			SatNotFoundException, ParseException {

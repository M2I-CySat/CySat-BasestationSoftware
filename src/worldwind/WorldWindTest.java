...
public class WorldWindTest {
	...
	/**
	 * Calculates a satellite's position given a starting position, inclination, elevation, and an angle along the orbit (all in degrees)
	 * 
	 * @param startLat
	 ...
	 * @return The position of the satellite at that angle along its orbit
	 */
	private static Position calcPosition(double startLat, double startLon, double inc, double theta, double alt) {
		...
		double lat2 = Math.asin(Math.sin(lat1) * Math.cos(thetar) + Math.cos(lat1) * Math.sin(thetar) * Math.cos(brng));
		double lon2 = lon1
				+ Math.atan2(Math.sin(brng) * Math.sin(thetar) * Math.cos(lat1), Math.cos(thetar) - Math.sin(lat1) * Math.sin(lat2));
	...
	private static Position calcPosition(Orbit orbit, double theta) {
		return calcPosition(orbit.getStartLatitude(), orbit.getStartLongitude(), orbit.getInclination(), theta, getAltitude(orbit, theta));
	}
